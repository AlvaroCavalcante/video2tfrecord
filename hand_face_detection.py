...
def compute_triangle_features(centroids, img, draw_on_img):
    euclidian_distances, img = compute_centroids_distances(img, draw_on_img, centroids)
...
def compute_features_and_draw_lines(bouding_boxes, img, draw_on_img=False):
    ...
    triangle_features = {}
    if len(centroids) == 3:
        triangle_features, img = compute_triangle_features(centroids, img, draw_on_img)
...
def filter_boxes_and_draw(image_np_with_detections, label_map_path, scores, classes, boxes, heigth, width, draw_on_image=False):
    ...
    for i in np.where(scores > .4)[0]:
        ...
        output_bboxes.append({class_name: {'xmin': xmin, 'xmax': xmax, 'ymin': ymin, 'ymax': ymax}})
...
def detect_visual_cues_from_image(**kwargs):
    ...
    drawn_image, bouding_boxes = infer_images(input_image, kwargs.get('label_map_path'), kwargs.get('detect_fn'), kwargs.get('height'), kwargs.get('width'))
    drawn_image, triangle_features = compute_features_and_draw_lines(bouding_boxes, drawn_image)

    face_segment, hand_1, hand_2 = get_image_segments(input_image, bouding_boxes)
